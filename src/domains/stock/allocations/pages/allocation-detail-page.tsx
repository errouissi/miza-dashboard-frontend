 ...
 * ALSO surface `ALLOCATION_EXCEEDS_DEPOSIT_CAPACITY` — handled REACTIVELY
 * ONLY, by `ValidateAllocationDialog`'s own error-code resolution. NO
 * PROACTIVE CAPACITY HINT EXISTS: re-confirmed against backend commit
 * `9af5d00` that `available` capacity is exposed only inside this
 * exception's own `context`, on a genuine refusal — the backend has no
 * read of it ahead of time (BC-AA remains partially open). A PROACTIVE
 * restock-gate integration briefly lived here (M6 Phase 3) reading the
 * recipient manager's own `useGrattageRestockGateQuery` — REMOVED by the
 * same backend commit, which deleted the team-wide hard block
 * (`ALLOCATION_TEAM_HAS_OUTSTANDING_OBLIGATION`) that gate mirrored and
 * replaced it with numeric, settlement-aware capacity the backend's own
 * `computeGrattageRestockGate()` docblock now explicitly states is "NOT
 * AUTHORITATIVE" for a manager. See `model/allocation.ts`'s own docblock
 * for the full backend-side change. Agent Transfer's own equivalent gate
 * is unaffected — its backend contract did not change.
 *
