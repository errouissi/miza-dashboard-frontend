...
import { CreateAgentStockReturnPage } from "./create-agent-stock-return-page";

const API = "http://localhost/api/v1";
const PATH = "/stock/agent-stock-returns/new";
...
/** `store()`'s own flat envelope — `{"data": new AgentStockReturnResource(...)}`. */
const CREATE_RETURN_ENVELOPE = {
  data: {
    id: 900,
    return_number: "RET-900",
    status: "draft",
    montant: "0.00",
    notes: null,
    return_date: null,
...
function renderPage(initialPath: string = PATH) {
  const router = createMemoryRouter(
    [
      { path: PATH, element: <CreateAgentStockReturnPage /> },
...
beforeEach(() => {
  ...
  signInWith([PERMISSIONS.CREATE_AGENT_STOCK_RETURN, PERMISSIONS.VIEW_AGENTS]);
});
...
describe("rendering", () => {
  it("renders the backend-supported fields", async () => {
    ...
    expect(await screen.findByLabelText("Return number")).toBeInTheDocument();
...
describe("validation", () => {
  it("shows a required error for every field on an empty submit", async () => {
    ...
    expect(await screen.findByText("Return number is required.")).toBeInTheDocument();
...
describe("submission and success", () => {
  it("submits the exact wire field names, omitting blank optional fields", async () => {
    ...
    server.use(
      managersHandler(),
      subDataHandler(20, [{ id: 10, nom: "Alaoui", prenom: "Sara", status: "active" }]),
      http.post(`${API}/admin/agent-stock-returns`, async ({ request }) => {
        body = await request.json();
        return HttpResponse.json(CREATE_RETURN_ENVELOPE, { status: 201 });
      }),
    );
    renderPage();

    fireEvent.change(await screen.findByLabelText("Return number"), {
      target: { value: "RET-900" },
    });
    await selectManagerAndCommercial();
  ...
  it("includes notes and return_date only when filled in", async () => {
    ...
    await waitFor(() => expect(body.notes).toBe("Excess stock"));
    expect(body.return_date).toBe("2026-07-25");
  });

  it("navigates to the new return's own detail page on success", async () => {
    server.use(
      managersHandler(),
      subDataHandler(20, [{ id: 10, nom: "Alaoui", prenom: "Sara", status: "active" }]),
      http.post(`${API}/admin/agent-stock-returns`, () =>
        HttpResponse.json(CREATE_RETURN_ENVELOPE, { status: 201 }),
      ),
    );
    ...
    await waitFor(() =>
      expect(router.state.location.pathname).toBe("/stock/agent-stock-returns/900"),
    );
  });
});
...
describe("Cancel", () => {
  it("navigates to the list without submitting", async () => {
    ...
    await screen.findByLabelText("Return number");
    fireEvent.click(screen.getByRole("button", { name: "Cancel" }));

    expect(router.state.location.pathname).toBe(RETURNS_PATH);
...
describe("failure preserves entered values", () => {
  it("shows a general error and does not navigate on a server failure", async () => {
    ...
    expect(
      await screen.findByText(/something went wrong recording this stock return/i),
    ).toBeInTheDocument();
    expect(router.state.location.pathname).toBe(PATH);
    expect(screen.getByLabelText("Return number")).toHaveValue("RET-900");
  });

  it("maps a field-level 422 (e.g. a duplicate return number) to its own field", async () => {
    server.use(
      managersHandler(),
      subDataHandler(20, [{ id: 10, nom: "Alaoui", prenom: "Sara", status: "active" }]),
      http.post(`${API}/admin/agent-stock-returns`, () =>
        HttpResponse.json(
          {
            ...
            errors: { return_number: ["The return number has already been taken."] },
          },
          { status: 422 },
        ),
      ),
    ...
    expect(
      await screen.findByText("The return number has already been taken."),
    ).toBeInTheDocument();
  });
  ...
  it("shows a permission-specific message on a 403", async () => {
    ...
    expect(
      await screen.findByText(/you do not have permission to record a stock return/i),
    ).toBeInTheDocument();
  });
});
